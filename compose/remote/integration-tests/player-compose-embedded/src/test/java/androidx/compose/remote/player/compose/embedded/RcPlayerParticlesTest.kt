...
class RcPlayerParticlesTest {
    ...
    fun particleSimulationSeedsAndAdvancesThroughCoreBridge() {
        ...
        val docContext =
            RemoteComposeContextAndroid(100, 100, "particles", AndroidxRcPlatformServices()) {
                val particles =
                    writer.createParticles(
                        vars,
                        arrayOf(floatArrayOf(10f), floatArrayOf(20f)),
                        3,
                    )
    ...
    /**
     * ParticlesCompare (per-particle conditional pass, bridged to core): when the condition
     * (x - 50 > 0) fires, the update equations mutate the particle in place.
     */
    @Test
    ...
    private fun findParticlesCreate(operations: Collection<Operation>): ParticlesCreate? {
        for (op in operations) {
            ...
            if (op is Container) findParticlesCreate(op.list)?.let { return it }
        }
        return null
    }
}
