 ...
 * through the production legacy (View) player and the experimental embedded Compose player — each
 * against its own golden, plus a direct parity comparison between the two.
 */
@MediumTest
...
class ExperimentalPreviewScreenshotTest {
    ...
    private fun sampleDocumentBytes(): ByteArray = runBlocking {
                ...
                content = {
                    RemoteColumn(modifier = RemoteModifier.size(120.rdp).background(Color.White)) {
                        RemoteRow(modifier = RemoteModifier.padding(8.rdp)) {
                            RemoteBox(
                                modifier =
                                    RemoteModifier.size(40.rdp).background(Color(0xFFAA2222))
                            )
                            RemoteBox(
                                modifier =
                                    RemoteModifier.size(40.rdp).background(Color(0xFF22AA22))
                            )
                        }
                        RemoteBox(
                            modifier =
    ...
    @Composable
    private fun PreviewUnderTest(bytes: ByteArray, impl: PlayerImpl, tag: String) {
        Box(modifier = Modifier.size(120.dp).testTag(tag)) {
            ExperimentalRemoteDocumentPreview(document = bytes, playerImpl = impl)
        }
    }

    @Test
    fun documentPreviewThroughViewPlayer() {
        val bytes = sampleDocumentBytes()
        rule.setContent { PreviewUnderTest(bytes, PlayerImpl.JAVA, "view") }
        rule.waitForIdle()
        rule
            .onNodeWithTag("view")
            .captureToImage()
    ...
    fun documentPreviewThroughEmbeddedPlayer() {
        val bytes = sampleDocumentBytes()
        rule.setContent { PreviewUnderTest(bytes, PlayerImpl.COMPOSE, "embedded") }
        rule.waitForIdle()
        rule
    ...
    /**
     * Parity: both players render the same document to (nearly) the same pixels. Golden-free —
     * the two captures are compared directly with MSSIM, so this guards embedded-vs-View drift
     * even before goldens exist.
     */
    @Test
    fun viewAndEmbeddedPlayersRenderTheDocumentTheSame() {
        val bytes = sampleDocumentBytes()
        rule.setContent {
            RemotePlayersSideBySide(bytes)
        }
        rule.waitForIdle()
        ...
        viewBitmap.getPixels(viewPixels, 0, viewBitmap.width, 0, 0, viewBitmap.width, viewBitmap.height)
    ...
    @Composable
    private fun RemotePlayersSideBySide(bytes: ByteArray) {
        androidx.compose.foundation.layout.Column {
            PreviewUnderTest(bytes, PlayerImpl.JAVA, "view")
            PreviewUnderTest(bytes, PlayerImpl.COMPOSE, "embedded")
        }
    }
}
