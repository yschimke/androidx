 ...
 * `CollapsibleRowLayout.computeVisibleChildren` uses — highest priority first, no-priority
 * children defaulting to `Float.MAX_VALUE`), and, exactly like core's walk, children are kept in
 * that order until the first one that overflows the budget; everything after it collapses.
 * Survivors are placed in document order. Only the measurement source differs: core measures with
 * its own layout pass, here Compose owns measure/layout, so the core walk is replayed against the
 * Compose-measured sizes (measurables are assumed to align 1:1 with the component's children in
 * document order).
 */
@Composable
internal fun RcPlayerCollapsible(
    ...
    Layout(content = { RcPlayerChildren(layout) { Modifier } }, modifier = modifier) {
        ...
        val indexOfChild = HashMap<Component, Int>(children.size)
        children.forEachIndexed { index, child -> indexOfChild[child] = index }
