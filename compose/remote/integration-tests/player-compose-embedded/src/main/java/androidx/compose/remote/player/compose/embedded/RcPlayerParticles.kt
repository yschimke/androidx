 ...
 * restores everything after. Compose-driven reactivity is preserved by reading the frame-clock
 * time through the [GraphContext]: that registers this draw as an observer of the frame tick, so
 * it re-runs (and the core code re-steps the simulation) every frame while the player's frame loop
 * is alive (kept alive for documents containing particles — see `containsParticles` in RcPlayer).
 *
...
private inline fun DrawScope.withCorePaintContext(
    ...
    val paintContext = AndroidPaintContext(remoteContext, drawContext.canvas.nativeCanvas)
    ...
    val previousPaintContext = remoteContext.paintContext
    ...
    try {
        ...
        remoteContext.setPaintContext(previousPaintContext)
    }
}
