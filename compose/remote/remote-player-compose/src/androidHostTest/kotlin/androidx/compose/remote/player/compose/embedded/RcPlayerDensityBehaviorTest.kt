 ...
 *
 * The document declares a 16-unit padding (stored as a raw float). Rendered at a display density of 2:
 ...
 * PIXELS/LEGACY. Density behavior is invisible at density 1 (×density == ÷density), hence the explicit
 * density-2 override.
 */
...
class RcPlayerDensityBehaviorTest {
    ...
    private fun paddingInsetDp(document: CoreDocument): Float {
        ...
        rule.waitForIdle()
        val outerLeft = rule.onNodeWithContentDescription("outer").getUnclippedBoundsInRoot().left.value
        val innerLeft = rule.onNodeWithContentDescription("inner").getUnclippedBoundsInRoot().left.value
    ...
    fun legacyBehaviorTreatsPaddingAsPixels() {
        // LEGACY (the creation-compose default): padding is px → 8dp inset at density 2 (unchanged).
