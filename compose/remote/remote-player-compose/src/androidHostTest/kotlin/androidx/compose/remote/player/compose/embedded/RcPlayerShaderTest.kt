 ...
 *
 * Findings (see also HISTORY.md, "Pixel verification under Robolectric"):
 * - Compose's [androidx.compose.ui.test.captureToImage] does NOT work in this Robolectric setup —
 *   `forceRedraw` times out even for a plain `Box(Modifier.background(Red))` with no RcPlayer
 *   involved, with or without `@GraphicsMode(NATIVE)`. So the embedded player's *pixel* output
 *   cannot be verified here; that needs an instrumented/emulator test (connectedCheck) or a
 *   Robolectric screenshot library (Roborazzi/Paparazzi).
 * - With `@GraphicsMode(NATIVE)`, the underlying `android.graphics` pipeline DOES rasterize shaders
 *   ([androidGraphicsLinearGradientRasterizes]) — so shaders themselves work under Robolectric; the
 *   gap is purely Compose's capture path.
 * - The embedded player processes a shader/gradient document end to end without error
 *   ([embeddedPlayerComposesGradientDocument]) — it builds the gradient `Brush` and runs the draw
 *   path; only the final pixel read-back is unavailable here.
 */
...
class RcPlayerShaderTest {
    ...
    /**
     * Baseline: does the Robolectric NATIVE graphics pipeline actually rasterize a `Shader`? Draws a
     * horizontal red->blue `LinearGradient` straight to an `android.graphics.Bitmap` (no Compose, no
     * RcPlayer) and checks the ends differ in the expected direction. This is the "do shaders work in
     * Robolectric" question in its purest form.
     */
    @Test
    fun androidGraphicsLinearGradientRasterizes() {
        val w = 100
        val h = 100
        ...
        val left = bitmap.getPixel(w / 10, h / 2)
        val right = bitmap.getPixel(w - w / 10, h / 2)

        assert(android.graphics.Color.red(left) > android.graphics.Color.blue(left)) {
            "Expected red-dominant left, got #${Integer.toHexString(left)} — shaders may not rasterize here"
        }
        assert(android.graphics.Color.blue(right) > android.graphics.Color.red(right)) {
            "Expected blue-dominant right, got #${Integer.toHexString(right)}"
        }
        assert(left != right) { "Gradient ends identical — not actually a gradient" }
    }
    ...
    /**
     * Exercises the CLIP_RECT draw op (now dispatched via drawContext.canvas.clipRect): a canvas that
     * clips to a sub-rectangle and then draws a full-size rect must compose/lay out without throwing.
     * Pixel correctness of the clip isn't checkable here (see class doc), so this asserts the path
     * runs end to end.
     */
    @Test
    ...
    /**
     * Exercises the DRAW_TEXT (canvas text run) op, newly dispatched via the native canvas. A canvas
     * that draws a text run must compose/lay out without throwing. (Pixel correctness isn't checkable
     * here — see class doc. Layout `Text` is the separately-verified path.)
     */
    @Test
    ...
    /**
     * Exercises the DRAW_TEXT_ANCHOR op (anchored text via measured bounds): a canvas that draws text
     * centered about an anchor point must compose/lay out without throwing. (Anchor placement isn't
     * pixel-assertable here — see class doc.)
     */
    @Test
     ...
     * must compose/lay out without throwing. (Path shape isn't pixel-assertable here — see class doc.)
     */
    @Test
