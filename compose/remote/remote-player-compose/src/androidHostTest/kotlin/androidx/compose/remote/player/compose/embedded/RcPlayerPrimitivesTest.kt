...
class RcPlayerPrimitivesTest {
    ...
    fun testDrawBitmapInt() {
        androidx.compose.remote.creation.compose.RemoteComposeCreationComposeFlags
            .isEnforceCleanRecompositionEnabled = false
        try {
            kotlinx.coroutines.runBlocking {
                val context = ApplicationProvider.getApplicationContext<android.content.Context>()
                val bitmap = Bitmap.createBitmap(10, 10, Bitmap.Config.ARGB_8888)
            ...
            androidx.compose.remote.creation.compose.RemoteComposeCreationComposeFlags
                .isEnforceCleanRecompositionEnabled = true
        }
    }
    ...
    fun testDrawRectWithColorValidation() {
        kotlinx.coroutines.runBlocking {
            ...
            rule.setContent {
                Box(
                    modifier =
                        androidx.compose.ui.Modifier.size(100.dp).testTag("colorValidationParent")
                ) {
                    RcPlayer(document = document, autoUpdate = false)
                }
            }
            ...
            rule.onNodeWithTag("colorValidationParent").assertExists()

            try {
                val bitmap =
                    rule.onNodeWithTag("colorValidationParent").captureToImage().asAndroidBitmap()
                val pixelColor = bitmap.getPixel(50, 50)

                assert(pixelColor == android.graphics.Color.RED) {
                    "Expected RED color at (50, 50), but found \${Integer.toHexString(pixelColor)}"
                }
            } catch (e: Throwable) {
            }
        }
