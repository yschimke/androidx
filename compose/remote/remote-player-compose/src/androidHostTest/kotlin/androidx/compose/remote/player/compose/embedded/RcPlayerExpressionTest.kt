...
class RcPlayerExpressionTest {
    ...
    fun testRegisterStoreLoadViaImperativePath() {
        // 5 STORE_R0 3 LOAD_R0 ADD == 3 + 5 == 8 (registers need eval-order state -> imperative path).
        assertMatchesCore(
            5f,
    ...
    fun testNoiseFromIsDeterministicViaImperativePath() {
        // NOISE_FROM is a pure hash (no RNG state); the imperative path must match the core exactly.
    ...
    fun testArrayOpsResolveThroughCollectionsAccess() {
        // A_SUM / A_MAX / A_LEN over a float array supplied via a CollectionsAccess. The array id is a
        ...
        assertThat(parseRpn(floatArrayOf(arrayNan, AnimatedFloatExpression.A_SUM), emptyMap(), ca).eval())
            .isEqualTo(10f)
        assertThat(parseRpn(floatArrayOf(arrayNan, AnimatedFloatExpression.A_MAX), emptyMap(), ca).eval())
            .isEqualTo(5f)
        assertThat(parseRpn(floatArrayOf(arrayNan, AnimatedFloatExpression.A_LEN), emptyMap(), ca).eval())
            .isEqualTo(3f)
    }
}
