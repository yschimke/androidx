 ...
 * unhandled op is *silently dropped*), and the component dispatch in `RcPlayer` over layout managers.
 * So when remote-core registers a new operation, the embedded player can fall out of sync without any
 * signal (this is what `operation_coverage.md` tried to track by hand).
 *
 * This test snapshots the set of opcodes remote-core registers. When core adds or removes an op the
 * set changes and this test fails. On failure: make sure the new op is handled by the embedded player
 * (`RcPlayerDrawing.executeOperations` for a draw op, the component `when` in `RcPlayer` for a layout)
 * — or confirm it intentionally renders nothing — and then update [GOLDEN_OPCODES] below. The point is
 * that the decision is made consciously, not silently skipped.
 */
class EmbeddedOperationCoverageTest {
    ...
    companion object {
        ...
        private val GOLDEN_OPCODES =
            setOf(
                0, 2, 14, 16, 38, 39, 40, 42, 43, 44, 46, 47, 48, 49, 51, 52, 53, 54, 55, 56, 57, 58,
                59, 63, 64, 65, 66, 67, 80, 81, 83, 101, 102, 103, 107, 108, 123, 124, 125, 126, 127,
                128, 129, 130, 131, 133, 134, 135, 136, 137, 138, 139, 140, 142, 143, 144, 145, 146,
                147, 148, 149, 150, 151, 152, 153, 154, 155, 156, 157, 158, 159, 160, 161, 163, 164,
                165, 166, 167, 168, 170, 171, 172, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182,
                183, 184, 185, 186, 187, 188, 190, 191, 192, 193, 194, 196, 197, 198, 199, 200, 201,
                202, 203, 204, 205, 207, 208, 209, 210, 211, 212, 213, 214, 215, 216, 217, 218, 219,
