...
class RcPlayerInteractivityTest {
    ...
    // embedded player resolves IntegerExpressions reactively (rememberRemoteIntExpression): the click
    ...
    // when its input changes. The click value-changes `n`; the embedded player recomputes the derived
    // op inside its derivedStateOf resolver (rememberRemoteStringAsState over LocalValueOps), so the
    // displayed text changes from "0" to "7". Before derived values were made reactive, the text was
    ...
    // over 1s, linear) and asserts the embedded player grows the box across time samples. This is the
    // appearance-animation path, made to work by the Compose-native animation layer: the dimension
    // resolves the raw variable id, the expression carries a `mFloatAnimation`, so resolution routes
    ...
    fun testAnimationSupport() {
        kotlinx.coroutines.runBlocking {
                ...
                captureSingleRemoteDocument(
                        ...
                        content = {
                            ...
                            RemoteBox(
                                modifier =
                                    RemoteModifier.semantics {
                                            contentDescription = "animated".rs
                                        }
                                        .width(animatedSize)
                                        .height(animatedSize)
                            )
            ...
            fun widthOf() = boxNode.getUnclippedBoundsInRoot().let { it.right.value - it.left.value }
            ...
            assert(endWidth > midWidth) {
                "Expected end width ($endWidth) > mid width ($midWidth)"
            }
        }
    }

    // A time-driven size — width/height keyed to `TIME_IN_SEC * scale` — must grow as the clock
    // advances. This is the genuinely-supported, fully compose-native animation path and is what the
    ...
    // polling, no `Animatable` — the layout recomposes from the time state each frame, like Compose.
    ...
    // and is exercised by the player. Rendering through RcPlayer and asserting equal measured widths
    // proves the full path: capture -> FloatExpression -> dimension modifier ->
    // rememberRemoteExpression -> parseRpn -> eval -> layout. (Operator *math* is exhaustively checked
    // against the core evaluator in RcPlayerExpressionTest; this proves they drive layout when played.)
    @Test
    ...
    fun dimensionConstraintsClampMeasuredSize() {
        kotlinx.coroutines.runBlocking {
            ...
            assert(kotlin.math.abs(maxW - 50f) < 3f) { "max-constrained width $maxW, expected ~50dp" }
            assert(kotlin.math.abs(minW - 80f) < 3f) { "min-constrained width $minW, expected ~80dp" }
        }
    }

    // FlowLayout previously ignored maxItemsInEachRow (bare FlowRow). With it honored, 3 boxes at
    // maxItemsInEachRow=2 must wrap: f0/f1 on row one, f2 on row two. (Without the fix all three fit
    ...
    // resolved name + value. Verifies the host-action wiring (HostNamedActionOperation -> callback).
    @Test
    fun hostNamedActionInvokesCallback() {
        kotlinx.coroutines.runBlocking {
            ...
            assert((firedValue as? Number)?.toInt() == 7) {
                "Expected value 7, got $firedValue"
            }
        }
    }
    ...
    // must label the player for accessibility — exposed as a semantics contentDescription on the root.
    @Test
    ...
    // Scrolling publishes the live offset to the document's scroll-position variable, so expressions
    // bound to it react. A marker box's width is bound to that variable; after swiping the scrollable
    // area the marker must widen — proving the offset reached the variable (not just native scroll).
    @Test
                        ...
                        content = {
                            ...
                            androidx.compose.remote.creation.compose.layout.RemoteColumn(
                                ...
                                RemoteBox(
                                    ...
                                    androidx.compose.remote.creation.compose.layout.RemoteColumn {
                                        repeat(5) { RemoteBox(modifier = RemoteModifier.size(80.rdp)) }
                                    }
                                }
