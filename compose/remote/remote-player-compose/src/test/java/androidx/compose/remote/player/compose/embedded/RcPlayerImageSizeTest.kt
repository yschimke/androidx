...
/**
 * Security regression: an inline PNG whose *declared* dimensions are tiny but whose *actual* decoded
 * dimensions are huge (a decompression-bomb / dimension-spoof attack) must be rejected, not loaded.
 *
 ...
 * `RemoteBitmapDecoder`, which bounds-checks the declared-vs-actual size). So the attack is rejected
 * either at document load ("invalid size") or when `RcPlayer` composes ("dimensions don't match").
 *
