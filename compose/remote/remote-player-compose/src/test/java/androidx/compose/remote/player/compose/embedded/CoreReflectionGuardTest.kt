...
/**
 * Fail-fast guard for the embedded player's reflective access to package-private remote-core fields.
 *
 * The player reads ~50 package-private fields on core operations by reflection (no public accessors
 * yet). A core rename would otherwise break the player *silently at runtime*, op by op. This test is
 * the central registry of that contract: it resolves every (class, field) pair and fails loudly here
 * if any is gone — turning a silent runtime regression into a build failure.
 *
