...
internal fun Modifier.width(op: WidthModifierOperation): Modifier {
    ...
    return when (op.type) {
        ...
        DimensionModifierOperation.Type.EXACT_DP -> {
            ...
            // rememberRemoteFloatAsState yields a non-reactive snapshot that never tracks the source.
            ...
            // sizes update like normal Compose. (`mValue` is package private in remote-core, which we
...
/** Cached reflective accessor for [DimensionConstraintsModifierOperation.mType] (no public getter). */
...
internal fun Modifier.dimensionConstraints(op: DimensionConstraintsModifierOperation): Modifier {
    ...
    return when (dimensionConstraintsTypeField.getInt(op)) {
...
/** Cached reflective accessor for [DimensionModifierOperation.mValue] (the raw source value). */
private val dimensionMValueField =
    DimensionModifierOperation::class.java.getDeclaredField("mValue").apply { isAccessible = true }

/**
 * The raw source value of a dimension op (`mValue`) — a NaN-encoded variable id for variable/
 * animation/time-backed dimensions, or a plain constant otherwise. Prefer this over
 * [DimensionModifierOperation.getValue], which returns the core-flattened `mOutValue` with the id
 * discarded and is therefore not reactive.
 */
internal fun dimensionRawValue(op: DimensionModifierOperation): Float = dimensionMValueField.getFloat(op)
