 ...
 *    rather than mutated into the store, which makes the op a pure function for the `derivedStateOf`.
 *
 * The operations are used verbatim (no core changes). Non-scalar reads (objects/bitmaps/collections/
 * paths) fall through to the shared store via `super`. The capture/cycle bookkeeping is single-thread
 * (Compose UI read phases); nested chain evaluation is handled by save/restore.
 *
...
internal class GraphContext(
    ...
    // Capture bookkeeping is per-thread: `derivedStateOf` may be evaluated on whichever thread reads
    ...
    // ops write paths/shaders/collections) — those ops aren't read through the scalar resolvers, but
