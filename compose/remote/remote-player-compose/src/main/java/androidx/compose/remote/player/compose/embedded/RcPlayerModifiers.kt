...
import androidx.compose.remote.player.compose.embedded.modifier.semantics
    ...
    list.fastForEach { op ->
        modifier =
            when (op) {
                ...
                // plain Row/Column), and LayoutCompute is custom measure/position logic the embedded
...
private fun Modifier.visible(op: ComponentVisibilityOperation): Modifier {
    val visible by rememberRemoteIntAsState(op.mVisibilityId)
