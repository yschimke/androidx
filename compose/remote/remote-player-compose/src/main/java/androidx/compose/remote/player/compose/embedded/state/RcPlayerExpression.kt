 ...
 * family, VAR1..3, and array/collection ops. Reactive scalar variables are resolved from [varStates]
 * at eval time (so a wrapping `derivedStateOf` tracks them); data-variable / array ids and operators
 * pass through verbatim, and array ops resolve through [collections].
 *
 ...
 * cache, and the spline cache live on the held instance (matching the core's per-instance lifetime);
 * the RAND seed is the core's process-static `Random`, exactly as in the legacy View player.
 */
        ...
        if (v.isNaN()) {
            if (AnimatedFloatExpression.isMathOperator(v)) {
                ...
                when (opCode) {
                    ...
                    // Binary (pop 2). Semantics mirror AnimatedFloatExpression.opEval; `left` is the
...
public fun rememberRemoteExpression(id: Int): State<Float> {
    ...
    // Array/collection ops (A_*) resolve their data through the document context's CollectionsAccess.
    val collections = LocalRemoteContext.current.collectionsAccess

    val tree = remember(document, expr, collections) { parseRpn(expr.mSrcValue, varStates, collections) }
 ...
 *
 * The expression's *source* value — its RPN over its variables, evaluated by [rememberRemoteExpression]
 * and ignoring the animation — is taken as the animation target. A Compose [Animatable], seeded at the
 * authored initial value, animates toward that target with the spec's duration and easing whenever the
 * target changes. Animation is therefore driven entirely by Compose's frame clock (here, via
 * [Animatable.animateTo]); the player's frame loop and the core's per-frame animation math are not
 * involved. This is the "it's just a `State<Float>`" layer: it sidesteps the core treating an
 * appearance animation's initial value as its target on first evaluation, and gives a real,
 * interruptible, value-change animation when the source is reactive.
 *
