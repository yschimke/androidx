 ...
 * remote-core's FitBoxLayout ("only display the child that fits in the available space"). The chosen
 * child is centered, like a Box.
 *
 * Children are measured unbounded to get their natural size, then the first with
 * `width <= maxWidth && height <= maxHeight` wins. (Core hides the FitBox entirely when nothing
 * fits; here we fall back to the first child so the player isn't blank.) FitBox children are
 * normally wrap/fixed-size alternatives; a child that wants to fill its parent has no natural size
 * and isn't a meaningful FitBox candidate.
 */
@Composable
