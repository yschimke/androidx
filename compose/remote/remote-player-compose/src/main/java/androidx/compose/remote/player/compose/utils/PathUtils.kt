 ...
 * start, end)`): at the endpoints it
 * uses the source data verbatim; in between it linearly interpolates each command word (preserving
 * NaN command markers from path 1). Returns an empty path if either path's data is missing.
 */
internal fun RemoteComposeState.getTweenPath(
    path1Id: Int,
    path2Id: Int,
    tween: Float,
    start: Float,
