...
/** [ScrollModifierOperation.mPositionExpression] — the NaN-encoded id of the scroll-position var. */
private val scrollPositionField =
    ScrollModifierOperation::class.java.getDeclaredField("mPositionExpression").apply {
        isAccessible = true
    }

/** [TouchExpression]'s stop mode + spec — the scroll's settle/notch behavior. */
...
/**
 * The even-notch count of a scroll, or 0 for free scrolling. The scroll's `TouchExpression` (a child
 * op of the scroll modifier) carries `STOP_NOTCHES_EVEN` + `stopSpec = [notches, notchMax]`; the count
 * is a constant in the bytes (the range is taken from Compose's scrollState, since the document's
 * notchMax is a layout-computed variable the embedded player doesn't populate).
 */
private fun evenNotchCount(op: ScrollModifierOperation): Int {
    ...
    if (touchStopModeField.getInt(touch) != TouchExpression.STOP_NOTCHES_EVEN) return 0
    val spec = touchStopSpecField.get(touch) as? FloatArray ?: return 0
...
internal fun Modifier.scroll(op: ScrollModifierOperation): Modifier {
    ...
    // scroll position — progress indicators, parallax — react), and an even-notch stop spec is honored
    // with a snapping fling. The core works in pixels and scrollState.value is px, so it's written
    // directly.
    val positionId =
        remember(op) {
            val raw = scrollPositionField.getFloat(op)
