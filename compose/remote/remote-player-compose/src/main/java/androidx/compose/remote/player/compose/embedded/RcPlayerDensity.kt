 ...
 * the `spacedBy` gap of `RowLayout`/`ColumnLayout`/`FlowLayout`, `MarqueeModifierOperation` spacing,
 * and `BorderModifierOperation` width. remote-core scales each of these by the display density only
 * under [CoreDocument.DENSITY_BEHAVIOR_DP] (in a layout/paint-time local that is never stored back);
 * under [CoreDocument.DENSITY_BEHAVIOR_PIXELS] and [CoreDocument.DENSITY_BEHAVIOR_LEGACY] the value is
 * used as raw pixels. Since a Compose [Dp] is re-multiplied by [density], a DP-mode value maps to
 * itself and a pixel-mode value is divided by [density] — reproducing the View player's pixels.
 *
