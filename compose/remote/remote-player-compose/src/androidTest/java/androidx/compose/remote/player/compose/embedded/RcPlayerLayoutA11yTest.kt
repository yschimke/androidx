...
/**
 * On-device layout + accessibility tests for the embedded [RcPlayer], driven with Compose UI testing
 * (Espresso-style semantics queries — no screenshots). These complement the Robolectric unit tests
 * by exercising the real device layout/measure + accessibility pipeline.
 */
...
class RcPlayerLayoutA11yTest {
    ...
    fun rootContentDescriptionLabelsThePlayer() {
        ...
        rule.setContent {
            Box(modifier = Modifier.size(200.dp)) { RcPlayer(document = document, autoUpdate = false) }
        }
        rule.waitForIdle()
    ...
    /**
     * Swiping a scrollable area publishes the live offset to the document's scroll-position variable
     * (a marker whose width is bound to that variable widens) — the device counterpart of the
     * Robolectric scroll-position test.
     */
    @Test
    fun scrollPublishesPositionToBoundVariable() {
        ...
        rule.setContent {
            Box(modifier = Modifier.size(300.dp)) { RcPlayer(document = document, autoUpdate = true) }
        }
        rule.waitForIdle()
