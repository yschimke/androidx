...
/**
 * A `Custom` (host-extension) component from a document: a config name plus its resolved properties,
 * keyed by property type. `Custom` components have no built-in rendering — the host app supplies it,
 * dispatched by [config]. This is the embedded-player equivalent of the View player's
 * `setCustomSupport`/`CustomContext`, surfaced as a Compose callback ([RcPlayer]'s `customContent`).
 *
 ...
 * computes a value and writes it back into the document so other ops react. Their property types are
 * listed in [floatReturns]/[textReturns]; the host writes via [returnFloat]/[returnText]. (For
 * convenience the current stored value of each return channel is also present in [floats]/[texts].)
 */
...
/**
 * App-provided renderer for [Custom] components, dispatched by [RcCustomComponent.config]. Defaults
 * to nothing (a Custom component renders empty unless the host supplies content). Provided by
 * [RcPlayer]'s `customContent` parameter.
 */
...
internal fun RcPlayerCustom(layout: Custom, modifier: Modifier) {
    ...
    val configId = customConfigIdField.getInt(layout)
    val config =
        if (configId != -1) remoteContext.getText(configId) ?: ""
        else (customConfigField.get(layout) as? String) ?: ""
    ...
    val properties =
        customPropertiesField.get(layout) as? List<Custom.CustomProperty> ?: emptyList()
    for (prop in properties) {
        ...
        when (prop.mDataType) {
            ...
            Custom.CustomProperty.TEXT_RETURN -> {
                ...
                when {
                    // Resolve a variable-backed float to its live value; a literal stays as authored.
                    prop.isFloat() ->
                        floats[type] = resolveFloat(prop.mFloatValue, prop.mFloatValue, remoteContext)
