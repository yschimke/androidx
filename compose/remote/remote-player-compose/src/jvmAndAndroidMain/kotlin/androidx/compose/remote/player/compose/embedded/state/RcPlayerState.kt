...
internal fun rememberRemoteStringAsState(id: Int): State<String> {
    ...
    // Derived text (TextFromFloat / TextMerge / TextLookup / …): resolve via the pure-Compose graph.
    ...
    return remember(document, id) {
        object : MutableState<String> {
            override var value: String
                // Read/write the snapshot-backed data store directly: the getter is reactive and the
...
internal fun rememberRemoteFloatAsState(id: Int): State<Float> {
    ...
    val expression = document.mFloatExpressions[id]
...
internal fun rememberRemoteLongAsState(id: Int): State<Long> {
    ...
    val state = remember(document, id) { mutableStateOf(initialValue) }
...
internal fun rememberMutableRemoteIntAsState(id: Int): MutableState<Int> {
    ...
    return remember(document, id) {
        object : MutableState<Int> {
            // Read/write the snapshot-backed integer store directly (no mirror/listener). The setter
            // uses overrideInteger so the value lands in the integer store the getter reads — the old
...
internal fun rememberNamedRemoteStringAsState(name: String): MutableState<String> {
    val document = LocalCoreDocument.current
    val context = document.remoteComposeState.remoteContext!!
    val id = context.getVariableId(name)
