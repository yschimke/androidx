...
internal fun Modifier.marquee(op: MarqueeModifierOperation): Modifier {
    ...
    // basic parameters to Compose's basicMarquee. The spacing may be a NaN-encoded variable/expression
    // (e.g. a dp recorded against the density variable), so resolve it before scaling; remote-core
    // scales it by density only under DP (rawDimensionDp). Velocity is not density-scaled.
    val density = LocalDensity.current.density
    val behavior = LocalCoreDocument.current.densityBehavior
    val spacing = rememberRemoteFloatAsState(op.mSpacing).value
    return this.basicMarquee(
        iterations = if (op.mIterations == -1) Int.MAX_VALUE else op.mIterations,
        animationMode =
            if (op.mAnimationMode == 0) MarqueeAnimationMode.Immediately
            else MarqueeAnimationMode.WhileFocused,
        repeatDelayMillis = op.mRepeatDelayMillis.toInt(),
        initialDelayMillis = op.mInitialDelayMillis.toInt(),
        spacing = MarqueeSpacing(rawDimensionDp(spacing, behavior, density)),
        velocity = op.mVelocity.dp,
    )
}
