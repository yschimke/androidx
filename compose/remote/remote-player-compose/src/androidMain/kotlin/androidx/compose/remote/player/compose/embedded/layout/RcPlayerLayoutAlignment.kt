...
/**
 * The 2D [Alignment] for a Box/FitBox `contentAlignment` from its horizontal + vertical positioning.
 * Expressed as a [BiasAlignment] (bias -1 = start/top, 0 = center, +1 = end/bottom), which is exactly
 * what the named `Alignment.TopStart`/`Center`/… constants are.
 */
...
/**
 * A Column's main-axis [Arrangement.Vertical] from its vertical positioning + `spacedBy` gap. The gap
 * is a raw value scaled per the document's density [behavior] (against [density]); see [rawDimensionDp].
 */
...
/**
 * A Row's main-axis [Arrangement.Horizontal] from its horizontal positioning + `spacedBy` gap. The gap
 * is a raw value scaled per the document's density [behavior] (against [density]); see [rawDimensionDp].
 */
...
// `mSpacedBy` (dp) is package-private in remote-core with no public accessor, and declared per
// layout class (not on the shared base). Read it via cached reflective fields rather than modifying
// remote-core. Resolved once at class load, then reused.
...
/** The `spacedBy` gap (dp) of a [RowLayout] (or subclass, e.g. FlowLayout). */
internal fun rowSpacedBy(layout: RowLayout): Float = rowSpacedByField.getFloat(layout)

/** The `spacedBy` gap (dp) of a [ColumnLayout]. */
