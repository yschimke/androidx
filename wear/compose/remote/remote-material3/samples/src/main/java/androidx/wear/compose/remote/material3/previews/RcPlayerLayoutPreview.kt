...
// Individual-feature confirmation previews (tranche: P0 layout/sizing/text + P1 border/offset/alpha)
...
private fun RcPaddingPreview(
    ...
    RemoteContentPreview(playerImpl = playerImpl) {
        Frame {
            RemoteBox(
                modifier =
                    RemoteModifier.size(160.rdp).background(Color(BLUE)).padding(24.rdp)
            ) {
...
private fun RcAlphaPreview(
    ...
    RemoteContentPreview(playerImpl = playerImpl) {
        Frame {
            RemoteBox(
                modifier =
                    RemoteModifier.size(120.rdp).alpha(0.4f.rf).background(Color(0xFF000000))
            )
        }
    }
